	...
	if ($self->safe_tie($id)) {
		if (defined($id)) {
			...
			if ($self->{object}->{tla} + $self->{object}->{ttl} < $self->now ||
			    $self->{check_ip} && $ENV{REMOTE_ADDR} ne $self->{object}->{ip}) {
...
sub restore {
	...
	if (defined($self->{old_session_id}) && !$self->{active}) {
			...
			assert($self->{old_session_id} eq $self->{data}->{_session_id}) if $self->{active};	
...
sub fresh {
	...
	my $self = shift;
	$self->destroy;
	return $self->try_open(undef);	# open a new one
}
1; # for require
