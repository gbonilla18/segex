...
sub GetCSV_head {
    ...
    my $search_terms =
      [ uniq_i split( /[,\s]+/, trim( car $q->param('q') ) ) ];
...
sub build_SearchPredicateGO {
    ...
    if ( $match eq 'Full-Word' ) {
        ...
        my @items = uniq_i split( /[,\s]+/, $self->{_QueryText} );
...
sub FindProbes_init {
    ...
    if ( $match eq 'Full-Word' ) {
        if ( my $p = $parser{$scope} ) {
            $text =~ s/^\W*//;
            $text =~ s/\W*$//;
            @textSplit = map { $p->($_) } uniq_i split( /[^\w:]+/, $text );
        }
        else {
            @textSplit = uniq_i split( /[,\s]+/, $text );
        }
    }
    else {
        @textSplit = uniq_i split( /[,\s]+/, $text );
...
sub build_SearchPredicate {
    ...
    if ( $match eq 'Full-Word' ) {
        if ( my $p = $parser{$scope} ) {
            ...
            $queryText =~ s/^\W*//;
            $queryText =~ s/\W*$//;
            my @items = map { $p->($_) } uniq_i split( /[^\w:]+/, $queryText );
            $self->{_QueryTextProc} = \@items;
...
sub printFindProbeCSV {
    ...
    my @queryItems = uniq_i split( /[,\s]+/, $search_params->{query_text} );
...
END_table_gene
        ...
        $self->{_QueryTextProc} = [ uniq_i split( /[,\s]+/, $self->{_QueryText} ) ];
    }
    else {
