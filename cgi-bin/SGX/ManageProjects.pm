...
sub new {
    ...
    my $self = {
		...
		_UserQuery  => 'SELECT uid, CONCAT(uname ,\' \\\\ \', full_name) FROM users WHERE email_confirmed',
        ...
        _LoadSingleQuery=> 'SELECT prid, prname, prdesc FROM project WHERE prid=?',
		# _UpdateQuery: update project description
        _UpdateQuery    => 'UPDATE project SET prname=?, prdesc=?, manager=? WHERE prid=?',
		# _InsertQuery: insert into project (what about ProjectStudy?)
        _InsertQuery    => 'INSERT INTO project (prname, prdesc, manager) VALUES (?, ?, ?)',
        ...
        _AddExistingStudy         => 'INSERT INTO ProjectStudy (prid,stid) VALUES (?, ?);',
...
sub loadAllStudiesFromProject
{
    ...
    $self->{_StudyRecordCount} = $self->{_StudyRecords}->execute($self->{_prid})
	   	or die $self->{_dbh}->errstr;
...
sub loadFromForm
{
    ...
    $self->{_SelectedStudy} = ($self->{_FormObject}->param('study_exist_unassigned'))
...
sub showProjects
{
    ...
    my $JSProjectList = "var JSProjectList =
    {
    ...
    print    '<div><a id="ProjectTable_astext" onClick = "export_table(JSProjectList)">View as plain text</a></div>' . "\n";
    print    '<div id="ProjectTable"></div>' . "\n";
    print    "<script type=\"text/javascript\">\n";
    print $JSProjectList;

    printTableInfo($self->{_FieldNames},$self->{_FormObject});
    printExportTable();
    ...
    print $self->{_FormObject}->start_form(
        -method=>'POST',
        -action=>$self->{_FormObject}->url(-absolute=>1).'?a=manageProjects&ManageAction=add',
        -onsubmit=>'return validate_fields(this, [\'name\']);'
    ) .
    $self->{_FormObject}->dl(
        $self->{_FormObject}->dt('name'),
        $self->{_FormObject}->dd($self->{_FormObject}->textfield(-name=>'name',-id=>'name',-maxlength=>255)),
        $self->{_FormObject}->dt('description:'),
        $self->{_FormObject}->dd($self->{_FormObject}->textarea(-name=>'description',-id=>'description',-rows=>8,-columns=>50,-maxlength=>1023)),
        $self->{_FormObject}->dt('&nbsp;'),
        $self->{_FormObject}->dd($self->{_FormObject}->submit(-name=>'AddProject',-id=>'AddProject',-value=>'Add Project'),$self->{_FormObject}->span({-class=>'separator'},' / ')
        )
    ) .
...
sub printTableInfo
{
    ...
    my @names     = @$arrayRef;
    my $CGIRef     = shift;
    my $deleteURL     = $CGIRef->url(absolute=>1).'?a=manageProjects&ManageAction=delete&id=';
    my $editURL    = $CGIRef->url(absolute=>1).'?a=manageProjects&ManageAction=edit&id=';

    #This is the code to use the AJAXy update box for description..
    my $postBackURL = '"'.$CGIRef->url(-absolute=>1).'?a=updateCell"';
    my $postBackQueryParametersDesc = '"type=project&name=" + escape(record.getData("0")) + "&desc=" + escape(newValue) + "&old_name=" + encodeURI(record.getData("0"))';
    my $postBackQueryParametersName = '"type=project&name=" + escape(newValue) + "&desc=" + escape(record.getData("1")) + "&old_name=" + encodeURI(record.getData("0"))';
    my $textCellEditorObjectDescr    = new SGX::DrawingJavaScript($postBackURL, $postBackQueryParametersDesc);
    my $textCellEditorObjectName    = new SGX::DrawingJavaScript($postBackURL, $postBackQueryParametersName);
    ...
    print    '
        ...
        var myColumnDefs = [
		{key:"0", sortable:true, resizeable:true, label:"Name", editor:' . $textCellEditorObjectName->printTextCellEditorCode() . '},
        {key:"1", sortable:true, resizeable:true, label:"Description",editor:' . $textCellEditorObjectDescr->printTextCellEditorCode() . '},
...
sub insertNewProject
{
    ...
    $self->{_dbh}->do($self->{_InsertQuery}, 
					  undef,
					  $self->{_prname},
					  $self->{_prdesc},
				  	  $self->{_manager})
	...
	warn "Inserted id: " . $self->{_dbh}->{'mysql_insertid'};	
...
sub deleteProject
{
	...
	$self->{_dbh}->do($self->{_DeleteQuery},
					  undef,
					  $self->{_prid})
		or die $self->{_dbh}->errstr;
}
...
sub removeStudy
{
    ...
    $self->{_dbh}->do($self->{_RemoveStudy},
					 undef,
					 $self->{_prid},
					 $self->{_delete_stid})
		or die $self->{_dbh}->errstr;
...
sub editProject
{
    ...
    print $self->{_FormObject}->start_form(
        -method=>'POST',
        -action=>$self->{_FormObject}->url(-absolute=>1).'?a=manageProjects&ManageAction=editSubmit&id=' . $self->{_prid},
        -onsubmit=>'return validate_fields(this, [\'description\']);'
    ) .
    $self->{_FormObject}->dl
        (
        $self->{_FormObject}->dt('name:'),
        $self->{_FormObject}->dd($self->{_FormObject}->textfield(-name=>'prname',-id=>'prname',-maxlength=>255,-value=>$self->{_prname})),
        $self->{_FormObject}->dt('description:'),
        $self->{_FormObject}->dd($self->{_FormObject}->textarea(-name=>'prdesc',-id=>'prdesc',-maxlength=>1023,-rows=>8,-columns=>50,-value=>$self->{_prdesc})),
        $self->{_FormObject}->dt('project manager:'),
        $self->{_FormObject}->dd($self->{_FormObject}->popup_menu(-name=>'manager',-id=>'manager',-values=>\@userListKeys,-labels=>\%{$self->{_userList}}, -default=>$self->{_mgr})),
        $self->{_FormObject}->dt('&nbsp;'),
        $self->{_FormObject}->dd($self->{_FormObject}->submit(-name=>'editSaveProject',-id=>'editSaveProject',-value=>'Save Edits'),$self->{_FormObject}->span({-class=>'separator'})
        )
    );

    my $JSStudyList = "var JSStudyList =
    {
        records: [". printJSStudyRecords($self) ."],
        headers: [". printJSStudyHeaders($self) . "]
    };" . "\n";
    ...
    printStudyTableInfo($self->{_StudyFieldNames},$self->{_FormObject},$self->{_prid});
	printDrawStudyResultsTableJS();

    print     "</script>\n";
    print $self->{_FormObject}->end_form;

    print    '<script src="' . $self->{_js_dir} . '/js/AddExistingStudy.js" type="text/javascript"></script>';
    print    '<br /><h2 name = "Add_Caption" id = "Add_Caption">Add Existing Study to this Project</h2>' . "\n";


    print    '<br /><h2 name = "Add_Caption1" id = "Add_Caption1">Studies in other projects.</h2>' . "\n";
    print     "<script>\n";
    ...
    print $self->{_FormObject}->start_form(
        ...
        -action=>$self->{_FormObject}->url(-absolute=>1).'?a=manageProjects&ManageAction=addExisting&id=' . $self->{_prid},
        -onsubmit=>"return validate_fields(this,'');"
    ) .
    $self->{_FormObject}->dl(
        $self->{_FormObject}->dt('Project : '),
        $self->{_FormObject}->dd($self->{_FormObject}->popup_menu(-name=>'project_exist', -id=>'project_exist',-values=>\@{$self->{_ExistingProjectValue}},-labels=>\%{$self->{_ExistingProjectList}},-default=>$self->{_SelectedProject}, -onChange=>"populateSelectExistingStudies(document.getElementById(\"study_exist\"),document.getElementById(\"project_exist\"));")),
        $self->{_FormObject}->dt('Study : '),
        $self->{_FormObject}->dd($self->{_FormObject}->popup_menu(-name=>'study_exist', -id=>'study_exist',-values=>\@{$self->{_ExistingStudyValue}}, -labels=>\%{$self->{_ExistingStudyList}},-default=>$self->{_SelectedStudy})),
        $self->{_FormObject}->dt('&nbsp;'),
    ...
    #
    print    '<br /><h2 name = "Add_Caption2" id = "Add_Caption2">Studies not in a project.</h2>' . "\n";
    ...
    $self->{_FormObject}->dl(
        $self->{_FormObject}->dt('Study : '),
        $self->{_FormObject}->dd($self->{_FormObject}->popup_menu(-name=>'study_exist_unassigned', -id=>'study_exist_unassigned',-values=>\@unassignedValues, -labels=>\%{$self->{_unassignedList}})),
        $self->{_FormObject}->dt('&nbsp;'),
        $self->{_FormObject}->dd($self->{_FormObject}->submit(-name=>'AddStudy',-id=>'AddStudy',-value=>'Add Study'),$self->{_FormObject}->span({-class=>'separator'},' / ')
        )
    ) .
...
#===  CLASS METHOD  ============================================================
#        CLASS:  ManageProjects
#       METHOD:  printDrawStudyResultsTableJS
#   PARAMETERS:  ????
#      RETURNS:  ????
...
#     SEE ALSO:  n/a
#===============================================================================
sub printDrawStudyResultsTableJS
{
    print    '
...
sub addExistingStudy
{
    ...
    my $sth = $self->{_dbh}->prepare($self->{_AddExistingStudy}) 
		or die $self->{_dbh}->errstr;
    $sth->execute($self->{_prid}, $self->{_SelectedStudy}) 
		or die $self->{_dbh}->errstr;
}

#===  CLASS METHOD  ============================================================
#        CLASS:  ManageProjects
#       METHOD:  printStudyTableInfo
#   PARAMETERS:  ????
#      RETURNS:  ????
...
#     SEE ALSO:  n/a
#===============================================================================
sub printStudyTableInfo
{
    my $arrayRef     = shift;
    my @names     = @$arrayRef;
    my $CGIRef     = shift;
    my $projectID    = shift;
    my $deleteURL     = $CGIRef->url(absolute=>1).'?a=manageProjects&ManageAction=deleteStudy&id=' . $projectID . '&removstid=';
    ...
    print    '
        ...
        {key:"2", sortable:false, resizeable:true, label:"Pubmed ID"},
        {key:"3", sortable:false, resizeable:true, label:"Platform"},
...
sub buildUnassignedStudyDropDown
{
    ...
    my $unassignedDropDown = new SGX::DropDownData(
		$self->{_dbh},
...
#===  CLASS METHOD  ============================================================
#        CLASS:  ManageProjects
#       METHOD:  printJavaScriptRecordsForExistingDropDowns
#   PARAMETERS:  ????
#      RETURNS:  ????
...
sub printJavaScriptRecordsForExistingDropDowns
{
    ...
    while (my @row = $sth->fetchrow_array) {
        ...
        $out .= 'project['.$row[0]."] = {};\n";
        $out .= 'project['.$row[0].'][0] = \''.$row[1]."';\n"; # project description
        $out .= 'project['.$row[0]."][1] = {};\n";     # sample 1 name
        $out .= 'project['.$row[0]."][2] = {};\n";     # sample 2 name
        $out .= 'project['.$row[0].'][3] = \''.$row[2]."';\n"; # user id
    }
    $sth->finish;
    ...
    while (my @row = $sth->fetchrow_array) {
        $out .= 'project['.$row[0].'][1]['.$row[1].'] = \''.$row[2]."';\n";
        $out .= 'project['.$row[0].'][2]['.$row[1].'] = \''.$row[3]."';\n";
    }
    $sth->finish;

    print $out;
    print 'function init() {';
    print 'populateExistingProject("project_exist");';
    print 'populateSelectExistingStudies(document.getElementById("study_exist"),document.getElementById("project_exist"));';
    print '}';
}
...
sub editSubmitProject
{
    ...
    $self->{_dbh}->do($self->{_UpdateQuery},
					  undef,
					  $self->{_name},
					  $self->{_prdesc},
					  $self->{_manager},
